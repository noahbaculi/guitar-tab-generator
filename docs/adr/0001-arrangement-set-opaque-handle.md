...
- The codebase otherwise uses `tsify-next` (with `serde-wasm-bindgen` pulled in transitively) for boundary crossings. `ArrangementSet` is the documented exception; `TabInput`, `NormalizedBeat`, and `TabError` keep the serde-via-tsify path. The rendered tab crosses as a bare `String`, not a serde-serialized type.
