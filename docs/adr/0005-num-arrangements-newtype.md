...
- The boundary error message ("must be between 1 and {max} inclusive, got {value}", with `max` = 20) is generated by `try_new` so direct Rust callers and WASM callers share it.
